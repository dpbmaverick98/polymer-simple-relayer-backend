...
try {
  ...
  // Try to connect to the relayer database in the parent directory
  const dbPath = join(__dirname, '../../relayer.db');
  
  if (existsSync(dbPath)) {
    db = new Database(dbPath);
    console.log('✅ Connected to relayer SQLite database at:', dbPath);
  } else {
    console.log('⚠️  Relayer database not found, creating demo database...');
    throw new Error('Database not found');
  }
} catch (error) {
  console.log('📝 Creating in-memory demo database with sample data...');
  ...
  try {
    ...
    console.error('❌ Failed to create demo database:', dbError);
    console.log('💡 Please install better-sqlite3: cd ui/server && npm install');
    process.exit(1);
  }
}
...
// Health check (should be first to test server)
...
app.listen(port, () => {
  ...
  console.log(`🔍 Test server: curl http://localhost:${port}/api/health`);
});
